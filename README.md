...
4. Скопируй полученный **BOT_TOKEN** (например: `2201663460:AAFEvojHympVcu9IIYDEUJYeJhGCbtN4fqo`)
...
2. Открой `.env.dev` и заполни следующие переменные:
   ...
   # Frontend - URL из cloudflared tunnel (терминал 1)
   VITE_GRPC_HOST=https://ваш-tunnel-url-для-backend.trycloudflare.com

   # WebApp URL - URL из cloudflared tunnel (терминал 2)
...
1. **Backend**: Откройте в браузере ваш backend tunnel URL - должна открыться страница с информацией о gRPC сервере
2. **Frontend**: Откройте в браузере ваш frontend tunnel URL - должно загрузиться приложение
...
1. Обнови `VITE_GRPC_HOST` и `WEBAPP_URL` в `.env.dev`
